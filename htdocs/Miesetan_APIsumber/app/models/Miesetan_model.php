...
class Miesetan_model extends MY_Model {
    ...
    public function getPenjualanprodukhari(){
        $query = $this->db->query("
            SELECT (SELECT nama from d_produk p where p.id = f.id_produk) as produk,DayName(tanggal)hari,SUM(total_pendapatan_produk)as total FROM `fakta_penjualan` f where Dayname(tanggal)= 'friday' GROUP by hari,produk
            ");
        return $query;
    }
