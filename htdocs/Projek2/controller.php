...
class controller{
	...
	public function insertdataprofil(){
		...
		if($fileName!=''&&$back==''){	
		$cek  = $this->model->tambahprofil($profil['id_user'],$username, $email,$date,$fileName,$profil['back']);			  		
		$cek2 = $this->model->updateuser($profil['id_user'],$username,$password,$email);
		}
		elseif($fileName==''&&$back!=''){
		$cek  = $this->model->tambahprofil($profil['id_user'],$username, $email,$date,$profil['photo'],$back);			  		
		$cek2 = $this->model->updateuser($profil['id_user'],$username,$password,$email);
		}
		elseif($fileName!=''&&$back!=''){
	...
	public function view_index()
	{
		...
		$jumlahfotoquery  = $this->model->jumlahfoto_user($profil['username']);
		$jumlahfoto = $this->model->fetch($jumlahfotoquery); 
	...
	{
		$userquery  = $this->model->user_id($id); 			
		$userdata = $this->model->fetch($userquery);									
		include 'Edit.php';
	...
	public function edituser(){
		if(isset($_POST['save'])){						
			$edituser = $this->model->updateuser($_POST['id_user'],$_POST['username'],$_POST['password'],$_POST['email']);			
			header("location:index.php");					
			exit();
		}	
	}
	
	public function crudfoto($user)
	{
		$fotoquery = $this->model->semua_fotouser($user);
		$fotodata = $this->model->fetch($fotoquery);
		
		$jumlahfotoquery  = $this->model->jumlahfoto_user($fotodata['username']);
		$jumlahfoto = $this->model->fetch($jumlahfotoquery);		
		include 'crudfoto.php';
	}
	...
	public function view_tambahuser()
	{
	...
	public function hapus(){
		...
		$hapus = $this->model->hapusfoto($_POST['hidden_id']);
		header("Refresh:0");
		exit();				
		}
	}
